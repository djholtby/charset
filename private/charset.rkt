...
(provide mib->charset-name charset-name->mib string->mib string->charset-name mib-number? charset-name?)
...
(define (mib-number? v)
  (and (exact-nonnegative-integer? v)
       (hash-has-key? charset-name-table v)))

(define (charset-name? v)
  (and (symbol? v)
       (hash-has-key? charset-value-table v)))

(define (mib->charset-name mib)
  (hash-ref charset-name-table mib #f))

(define (charset-name->mib cn)
  (hash-ref charset-value-table cn #f))

(define (string->mib str)
  (hash-ref charset-value-table (string->symbol (string-upcase str)) #f))

(define (string->charset-name str)
  (mib->charset-name (string->mib str)))
